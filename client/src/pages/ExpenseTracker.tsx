...
const ExpenseTracker: React.FC = () => {
  ...
  const [expenses, setExpenses] = useState([
    { id: '1', date: '2026-05-10', category: 'Printing', description: 'Flex printing for Reliance campaign', amount: '₹45,000', vendor: 'Digital Flex Printers', status: 'Paid' },
    { id: '2', date: '2026-05-08', category: 'Lease', description: 'Monthly rent for Ambala site', amount: '₹22,000', vendor: 'Haryana Outdoor Media', status: 'Pending' },
    { id: '3', date: '2026-05-05', category: 'Salaries', description: 'May staff salaries', amount: '₹1,20,000', vendor: 'Internal', status: 'Paid' },
  ]);
         ...
         <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-border">
               {filteredExpenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-bg-surface-2 transition-colors">
                     ...
                     <td className="px-6 py-4 text-center">
                        <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full text-white ${exp.status === 'Paid' ? 'bg-success' : 'bg-warning'}`}>
                           {exp.status}
                        </span>
                     </td>
                     <td className="px-6 py-4 text-right">
                        <div className="text-[13px] font-black text-danger">{exp.amount}</div>
                     </td>
                  </tr>
               ))}
            </tbody>
        ...
        <div className="fixed inset-0 bg-bg-primary/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
           <div className="bg-bg-surface border border-border rounded-2xl w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="p-6 border-b border-border flex justify-between items-center bg-bg-surface-2">
                 <h2 className="text-lg font-bold flex items-center gap-2 text-text-primary"><Plus size={18} className="text-accent-orange" /> Log New Expense</h2>
                 <button onClick={() => setShowAddModal(false)} className="p-2 hover:bg-bg-surface-2 rounded-lg transition-colors">X</button>
              </div>
              <div className="p-6 space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5"><label className="text-[10px] font-black text-text-muted uppercase">Vendor</label><select className="w-full bg-bg-surface-2 border border-border rounded-xl px-3 py-2.5 text-[12px] outline-none"><option>Haryana Outdoor Media</option><option>Digital Flex Printers</option><option>Internal</option></select></div>
                 </div>
              </div>
              <div className="p-6 border-t border-border flex justify-end gap-3 bg-bg-surface-2">
                 ...
                 <button onClick={() => { toast.success('Expense recorded!'); setShowAddModal(false); }} className="btn-primary px-8 py-2 text-[12px]">Save Entry</button>
              </div>
           </div>
        </div>
      )}
