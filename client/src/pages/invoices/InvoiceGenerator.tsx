...
const InvoiceGenerator: React.FC = () => {
  ...
  const [docMode, setDocMode] = useState<'invoice' | 'quotation'>(location.state?.defaultDocMode || 'invoice');
  ...
  const getNextAutoNumber = useCallback((existingInvoices: any[], mode: 'invoice' | 'quotation' = 'invoice') => {
    const today = new Date();
    const year = today.getFullYear();
    const prefix = mode === 'quotation' ? `QUO-${year}-` : `INV-${year}-`;
    ...
    try {
      ...
      if (id) {
        try {
          ...
          setDocMode(inv.status === 'QUOTATION' || (inv.invoiceNumber && inv.invoiceNumber.startsWith('QUO-')) ? 'quotation' : 'invoice');
        ...
        const nextAuto = getNextAutoNumber(invoicesRes.data || [], docMode);
  ...
  useEffect(() => {
    if (!id && orgId) {
      api.get('/invoices').then((res) => {
        const nextAuto = getNextAutoNumber(res.data || [], docMode);
        setAutoGeneratedNumber(nextAuto);
        if (!isCustomNumber) {
  ...
  const saveInvoice = async () => {
    ...
    try {
      ...
      toast.success(docMode === 'quotation' ? 'Quotation saved!' : 'Invoice saved!');
      navigate('/invoices');
    } catch (error: any) {
    ...
    <div className="flex flex-col h-screen bg-bg-primary overflow-hidden print:bg-white text-text-primary">
      <div className="h-14 bg-bg-surface border-b border-border flex items-center justify-between px-6 shrink-0 print:hidden">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/invoices')} className="p-2 hover:bg-bg-surface-2 rounded-lg transition-colors text-text-primary">
            <ArrowLeft size={20} />
          </button>
          <h1 className="text-sm font-black uppercase tracking-widest capitalize">{id ? `Edit ${docMode}` : `New ${docMode}`}</h1>
          {!id && (
            <div className="flex bg-bg-surface-2 p-0.5 rounded-lg border border-border ml-2">
              <button
                type="button"
                onClick={() => setDocMode('invoice')}
                className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${docMode === 'invoice' ? 'bg-accent-blue text-white shadow-sm' : 'text-text-muted hover:text-text-primary'}`}
              >
                Invoice
              </button>
              <button
                ...
                className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${docMode === 'quotation' ? 'bg-accent-blue text-white shadow-sm' : 'text-text-muted hover:text-text-primary'}`}
              >
                Quotation
              </button>
            </div>
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="bg-bg-surface-2 border border-border text-text-primary px-4 py-2 rounded-lg font-black text-xs uppercase tracking-widest hover:border-accent-orange transition-all flex items-center gap-2 disabled:opacity-50"
          >
