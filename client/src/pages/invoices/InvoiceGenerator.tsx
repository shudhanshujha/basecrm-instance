...
const InvoiceGenerator: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      setIsLoading(true);
      const [meRes, clientsRes, campaignsRes, sitesRes, invoicesRes] = await Promise.all([
        api.get('/auth/me'),
        api.get('/clients'),
        api.get('/campaigns'),
        api.get('/sites'),
      ...
      // If id is specified, we load that invoice instead of creating a new auto number
      if (id) {
        try {
          ...
          setFormData({
            ...
            upiId: inv.upiId || org.upiId || '7015177522@pnb',
            ...
            placeOfSupply: inv.placeOfSupply || 'HARYANA (06)',
            descriptionHeader: inv.notes || 'Advertising Hoarding Site Display',
            seller: {
            ...
            buyer: {
              ...
              stateCode: inv.client?.pincode ? '06' : '06'
            },
            ...
            terms: "1. Payment should be made by Cheque/DD/NEFT in favor of DRISHTI VISION SOLUTION.\n2. Interest @18% p.a. will be charged if payment is not made within due date.",
            showDigitalSignature: inv.showDigitalSignature !== null ? inv.showDigitalSignature : false,
            signatureUrl: inv.signatureUrl || '',
            showUpiQr: inv.showUpiQr !== null ? inv.showUpiQr : true
          });
        } catch (err) {
  ...
  const saveInvoice = async () => {
    try {
      ...
      if (id) {
        ...
        toast.success('Invoice updated in ledger!');
      } else {
      ...
      toast.error(id ? 'Failed to update invoice' : 'Failed to register invoice');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden print:block">
        ...
        <div className="w-full lg:w-1/2 overflow-y-auto p-8 border-r border-border bg-bg-primary custom-scrollbar print:hidden">
          <div className="space-y-8 max-w-2xl mx-auto pb-20">
            ...
            {/* Section 1.6: QR Code & Payment Options */}
            <div className="card space-y-6">
              <h2 className="text-[12px] font-black text-success uppercase tracking-[2px] border-b border-border pb-3">01c. QR Code & Payments</h2>
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 flex items-center gap-2">
                  <input 
                    ...
                    className="w-4 h-4 rounded border-border text-accent-orange focus:ring-accent-orange cursor-pointer"
                  />
                  <label htmlFor="showUpiQr" className="text-[11px] font-black text-text-primary uppercase cursor-pointer">
                    Enable UPI QR Code
                  </label>
                </div>
                {formData.showUpiQr && (
                  <div className="col-span-2 space-y-1.5">
                    <label className="text-[11px] font-black text-text-muted uppercase">UPI ID (VPA)</label>
                    <input 
                      type="text" 
                      className="w-full bg-bg-surface-2 border border-border rounded-xl px-3 py-2 text-[13px] outline-none" 
                      value={formData.upiId} 
                      onChange={e => setFormData({...formData, upiId: e.target.value})} 
                      placeholder="e.g. 7015177522@pnb"
                    />
                  </div>
                )}
                <div className="col-span-2 flex items-center gap-2">
                  <input 
                    ...
                    className="w-4 h-4 rounded border-border text-accent-orange focus:ring-accent-orange cursor-pointer"
                  />
                  <label htmlFor="showDigitalSignature" className="text-[11px] font-black text-text-primary uppercase cursor-pointer">
                    Enable Digital Signature
                  </label>
                </div>
                {formData.showDigitalSignature && (
                  <div className="col-span-2 space-y-1.5">
                    <label className="text-[11px] font-black text-text-muted uppercase">Signature Image URL</label>
                    <input 
                      type="text" 
                      className="w-full bg-bg-surface-2 border border-border rounded-xl px-3 py-2 text-[13px] outline-none" 
                      value={formData.signatureUrl} 
                      onChange={e => setFormData({...formData, signatureUrl: e.target.value})} 
                      placeholder="Enter signature image URL or path"
                    />
                  </div>
                )}
              </div>
