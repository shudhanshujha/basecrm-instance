    ...
    <div className="space-y-8 pb-12">
      <div className="flex items-center justify-between">
         ...
         <div className="flex gap-2">
            ...
            <button onClick={() => toast.success('Payout wizard initiated...')} className="btn-primary text-[15px] py-1.5 flex items-center gap-2 shadow-lg shadow-accent-orange/30"><Plus size={16} /> New Payout</button>
         </div>
      </div>

      <div className="grid grid-cols-4 gap-4">
         <div className="card flex flex-col justify-center gap-2">
            <div className="text-[12px] text-text-muted font-black uppercase tracking-tighter">Primary Contact</div>
            <div className="flex items-center gap-2 text-[16px] font-bold text-text-primary"><User size={14} className="text-accent-orange" /> {vendor.contactPerson}</div>
            <div className="flex items-center gap-2 text-[14px] text-text-muted font-bold"><Phone size={12} className="text-accent-blue" /> {vendor.phone}</div>
         </div>
         ...
         <div className="card border-warning/20 bg-warning/5">
            <div className="text-[12px] text-warning uppercase font-black tracking-widest">Verification Status</div>
            <div className="text-2xl font-black text-text-primary mt-2">Verified</div>
            <div className="text-[12px] text-text-muted font-bold mt-2 uppercase">Compliant Partner</div>
         </div>
      </div>
      ...
      <div className="card p-0 overflow-hidden border-border/50 shadow-xl">
         <div className="flex border-b border-border bg-bg-surface-2">
            {['contracts', 'payouts'].map((tab: any) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-8 py-4 text-[14px] font-black uppercase tracking-widest transition-all relative ${activeTab === tab ? 'text-accent-orange bg-accent-orange/5' : 'text-text-muted hover:text-text-primary'}`}>
                {tab}
            ...
            {activeTab === 'contracts' && (
               <div className="overflow-x-auto">
                  <table className="w-full text-left">
                     <thead>
                        <tr className="text-[13px] text-text-muted uppercase font-black border-b border-border">
                        ...
                        {vendor.vendorContracts?.length > 0 ? vendor.vendorContracts.map((c: any) => (
                           <tr key={c.id} className="group hover:bg-bg-surface-2 transition-colors">
                              <td className="py-4 px-4">
                                 <div className="text-[16px] font-bold text-text-primary uppercase">{c.id.slice(0, 8)}</div>
                                 <div className="text-[13px] text-text-muted font-mono mt-0.5 uppercase tracking-tighter">Created: {new Date(c.createdAt).toLocaleDateString()}</div>
                              </td>
                              <td className="py-4 px-4 text-[15px] text-text-muted font-medium uppercase">
                                {new Date(c.startDate).toLocaleDateString()} - {new Date(c.endDate).toLocaleDateString()}
                              </td>
                              <td className="py-4 px-4 font-mono text-[16px] font-black text-accent-blue text-right">₹{c.rate.toLocaleString()}</td>
                     ...
                     <tbody className="divide-y divide-border">
                        {vendor.vendorPayments?.length > 0 ? vendor.vendorPayments.map((p: any) => (
                           <tr key={p.id} className="group hover:bg-bg-surface-2 transition-colors">
                              <td className="py-4 px-4">
                                 <div className="text-[16px] font-bold text-text-primary uppercase">{p.referenceNumber || 'N/A'}</div>
                                 <div className="text-[13px] text-text-muted font-mono mt-0.5 uppercase tracking-tighter">{new Date(p.paymentDate).toLocaleDateString()}</div>
                              </td>
                              <td className="py-4 px-4 text-[15px] text-text-muted font-medium uppercase">{p.paymentMode}</td>
                              <td className="py-4 px-4 font-mono text-[16px] font-black text-accent-orange text-right">₹{p.amount.toLocaleString()}</td>
                           </tr>
                        )) : (
